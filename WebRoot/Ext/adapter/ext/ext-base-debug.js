...
/**
 * @class Ext Ext类
 * Ext core utilities and functions. 核心工具和函数
 * @singleton 单例模式
 */

Ext = {
    /**
     * The version of the framework 框架版本
     * @type String
     */
    version : '3.0'
};
...
/**
 * Copies all the properties of config to obj. 复制config的所有属性至obj
 * @param {Object} obj The receiver of the properties 属性的接收者
 * @param {Object} config The source of the properties 属性源
 * @param {Object} defaults A different object that will also be applied for default values 复制为默认值的对象
 * @return {Object} returns obj
 * @member Ext apply
 */
Ext.apply = function(o, c, defaults){
    // no "this" reference for friendly out of scope calls
    if(defaults){//默认属性,若c对象里有同名属性将被覆盖
        Ext.apply(o, defaults);
    }
    if(o && c && typeof c == 'object'){//c必须为object类型
        for(var p in c){//将原对象的同名属性值覆盖了
            o[p] = c[p];
        }
    }
    return o;
...
(function(){
	//定义一些参数及方法
    var idSeed = 0,//生成独有id种子,即计数器,局部变量
        toString = Object.prototype.toString,//获取Object原型的toString方法
        //assume it's not null and not an array
        isIterable = function(v){//参数是否可迭代,数组,结点对象数组,IXMLDOMNodeList
            //check for array or arguments 数组或参数
            if(Ext.isArray(v) || v.callee){
                return true;
            }
            //check for node list type 结点列表
            if(/NodeList|HTMLCollection/.test(toString.call(v))){
                return true;
            }
            //NodeList has an item and length property 结点列表拥有item,length属性
            //IXMLDOMNodeList has nextNode method, needs to be checked first. IXMLDOMNodeList有nextNode方法,需先检查
            return ((v.nextNode || v.item) && Ext.isNumber(v.length));
        },
        ua = navigator.userAgent.toLowerCase(),//获取浏览器类型字串,外部变量
        check = function(r){//测试ua是否存在r值或正则模式
            return r.test(ua);
        },
        DOC = document,//获取document的引用
        isStrict = DOC.compatMode == "CSS1Compat",//兼容模式
        isOpera = check(/opera/),
        ...
        isLinux = check(/linux/),
        isSecure = /^https/i.test(window.location.protocol);//是否使用安全协议https

    // remove css image flicker 若为ie6,缓存背景图片,避免同一张图片加载多次
    if(isIE6){
        try{
    ...
    Ext.apply(Ext, {//给Ext对象添加属性
		 ...
		 * 表示是否使用本地浏览器解析json的方法
         * This option is ignored if the browser does not support native JSON methods.
         * <b>Note: Native JSON methods will not work with objects that have functions. 如objcet有方法,本机JSON方法将不起作用
         * Also, property names must be quoted, otherwise the data will not parse.</b> (Defaults to false) 同时属性名必须加引号,否则不予解析
         * @type Boolean
         */
		 ...
		 * 复制config所有属性至obj,如果他们不曾出现在obj中,即不覆盖原有属性
         * @param {Object} obj The receiver of the properties 属性接收对象
         * @param {Object} config The source of the properties 属性源
         * @return {Object} returns obj
         */
		 ...
		 * 产生一个独有的id,如果元素已经有id,将不予处理
         * @param {Mixed} el (optional) The element to generate an id for 要产生id的元素
         * @param {String} prefix (optional) Id prefix (defaults "ext-gen") id前缀
...
MyGridPanel = Ext.extend(Ext.grid.GridPanel, {
    ...
    yourMethod: function() {
         ...
         * </p>
         * @param {Function} subclass The class inheriting the functionality 继承功能的类
         * @param {Function} superclass The class being extended 被扩展的父类
         * @param {Object} overrides (optional) A literal with members which are copied into the subclass's
		 * 一序列被复制至子类原型的成员,因此成员被新类所有实例共享
        ...
        extend : function(){
            // inline overrides
            var io = function(o){//将o对象属性复制至当前对象
                for(var m in o){
                    this[m] = o[m];
                }
            };
            var oc = Object.prototype.constructor;//根类构造函数

            return function(sb, sp, overrides){//sb为子类,sp父类
                if(Ext.isObject(sp)){
                    overrides = sp;
                    sp = sb;
...
Ext.override(MyClass, {
    ...
    newMethod2: function(foo){
         ...
         * @param {Object} origclass The class to override 要覆写的类
         * @param {Object} overrides The list of functions to add to origClass.  This should be specified as an object literal 增加至origClass的方法列表,应指定为一个对象序列
         * containing one or more methods.
         * @method override
         */
        override : function(origclass, overrides){
            if(overrides){
                var p = origclass.prototype;//通过原型链覆写,propotype是一个对象
...
Company.data.CustomStore = function(config) { ... }
         ...
         * @method namespace
         */
        namespace : function(){//创建命名空间,其实只是把相应属性放至window下
            var o, d;
        ...
        urlEncode: function(o, pre){
            ...
            for(key in o){
                undef = !Ext.isDefined(o[key]);//是否存在该键值
...
Ext.urlDecode("foo=1&bar=2&bar=3&bar=4", false); // returns {foo: "1", bar: ["2", "3", "4"]}
         ...
         * @param {Object} scope
         */
        each: function(array, fn, scope){//只能迭代数组类型的参数
         ...
         * @param {Object/Array} object The object or array to be iterated 要迭代的数组或对象
         * @param {Function} fn The function to be called for each iteration. 迭代项调用的方法
         ...
         */
        iterate : function(obj, fn, scope){//可迭代数组或对象
            if(isIterable(obj)){
...
function(el){
    ...
    // do something with the dom node
         ...
         */
        getDom : function(el){//0,'',undefined,null返回null
            if(!el || !DOC){
                return null;
            }
         ...
         * </ul></div>
         * @param {Mixed} value The value to test 测试值
         * @param {Boolean} allowBlank (optional) true to allow empty strings (defaults to false) 是否允许一个字串为空
         * @return {Boolean}
         */
        isEmpty : function(v, allowBlank){//null,undefined,empty Array,空字串(可用allowBlank控制)
            return v === null || v === undefined || ((Ext.isArray(v) && !v.length)) || (!allowBlank ? v === '' : false);
        },

        /**
         * Returns true if the passed object is a JavaScript array, otherwise false.
		 * 如果传递对象是一个js数组,则返回true
		 ...
		 * 如传递对象是js的一个基本类型:string,number,boolean,则返回true
         * @param {Mixed} value The value to test
         * @return {Boolean}
         */
        isPrimitive : function(v){//基本类型:字串,数字,布尔值
         ...
         */
        isNumber: function(v){
            return typeof v === 'number' && isFinite(v);//是否为有限值
        },
		 ...
		 * 如浏览器为Safari 3.x,则返回true
         * @type Boolean
         */
        isSafari3 : isSafari3,
        /**
         * True if the detected browser is Safari 4.x.
		 * 如浏览器为Safari 4.x,则返回true
         * @type Boolean
         */
        isSafari4 : isSafari4,
        /**
         * True if the detected browser is Safari 2.x.
		 * 如浏览器为Safari 2.x,则返回true
         * @type Boolean
         */
        isSafari2 : isSafari2,
        /**
		 ...
		 * 如浏览器使用gecko内核,则返回true
         * @type Boolean
         */
        isGecko : isGecko,
        /**
         * True if the detected browser uses a pre-Gecko 1.9 layout engine (e.g. Firefox 2.x).
		 * 如浏览器使用pre-gecko内核,则返回true
         * @type Boolean
         */
        isGecko2 : isGecko2,
        /**
         * True if the detected browser uses a Gecko 1.9+ layout engine (e.g. Firefox 3.x).
		 * 如浏览器使用pre-gecko 1.9+内核,则返回true
         * @type Boolean
         */
        isGecko3 : isGecko3,
        /**
         * True if the detected browser is Internet Explorer running in non-strict mode.
		 * 如浏览器为IE且禁止脚本执行,则返回true
         * @type Boolean
         */
		 ...
		 * 如操作系统为Max os,则返回true
         * @type Boolean
         */
        isMac : isMac,
        /**
         * True if the detected platform is Adobe Air.
		 * 如操作系统为Adobe Air,则返回true
         * @type Boolean
         */
        isAir : isAir
    });
...
sayHiToFriend('Brian'); // alerts "Hi, Brian"
     ...
     * @param {Function} fcn The function to call before the original 初始化前被调用的函数
     * @param {Object} scope (optional) The scope of the passed fcn (Defaults to scope of original function or window) 调用函数的作用域
 ...
 * @class Array
 */
Ext.applyIf(Array.prototype, {//添加方法至Array对象,已有则不覆写
    /**
