...
$(function(){
	  $("#J_setHomeBtn").click(function(e){
		...
		/*
		$.getJSON('userrealname.do?random=' + Math.random(), function(data){ 
			if(data.realname!=''&&data.realname!='undefined'&&data.realname!=null){
				$('#realname').html("您好："+data.realname ); 
		//			 window.location.href = "index.html";
				$(".after-login").css("display","block");
				$(".before-login").css("display","none");
				$('#avatarurl').attr("src","/SchoolManageSystem"+data.avatarurl);
				$("#edums").attr("href","http://113.98.255.56/edums/loginInteg.action?tickets="+data.md5str);
			}else{
		  ...
		  $.ajax({
				 ...
				 if(data.ui!='undefined'&&data.ui!=null){
					...
					if(data.ui.avatarurl!=''&&data.ui.avatarurl!=null){
						$('#avatarurl').attr("src","http://113.98.255.59/SchoolManageSystem"+data.ui.avatarurl);
					}else{
						$('#avatarurl').attr("src","../images/login/photo-head.jpg");
					}
					$(".after-login").css("display","block");
					$(".before-login").css("display","none");
					$("#edums").attr("href","http://113.98.255.56/edums/loginInteg.action?tickets="+data.ui.md5str);
					*/
//					 window.location.href = "paasindex.html";
				 	 window.location.href = "../manage/main.jspx";
					}
				 }
			  });
	  }
