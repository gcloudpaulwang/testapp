				...
				if (isNonEmpty(fileName))
				{
					...
					if (files != null && files.length > 0)
					{
						...
						for (int index = 0; index < files.length; index++)
							...
							{
								boolean unzip;
								if(request.getParameter("unzip")==null) unzip=false;
								else
								unzip=Boolean.getBoolean(request.getParameter("unzip"));
								if(!FilenameUtils.getExtension(fileName[index]).toLowerCase().equals("zip")&&unzip){
									baseAction.getJsonRoot().put("success", false);
									baseAction.getJsonRoot().put("exception", "不能解压非zip格式文件");
									return Action.SUCCESS;
								}
