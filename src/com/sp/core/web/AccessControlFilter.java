	...
	public void doFilter(ServletRequest servletRequest,
		...
		if (isControl) {
			...
			if(postfix.equals("jspx"))
				isJspxPostix=true;
			if (session == null||session.getAttribute(User.RIGHTS_KEY)==null) {
				if(isJspxPostix)
			...
			List<String> uris=  (List<String>)session.getAttribute(User.RIGHTS_KEY);
			if (uris == null || !uris.contains(url)) {
				
				if(isJspxPostix)
					resp.sendError(HttpServletResponse.SC_FORBIDDEN);
				if(isDoPostfix){
					resp.setCharacterEncoding("utf-8");
					resp.getWriter().write("{success:false,forbidden:true}");
				}
				return;
			}
		}else{
